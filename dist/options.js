...
var backupAllProfilesBtn = document.getElementById("backupAllProfiles");
...
async function backupAllProfiles() {
  try {
    ...
    const blob = new Blob([json], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const filename = `pushstr_profiles_backup_${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}.json`;
    const link = document.createElement("a");
    link.href = url;
    link.download = filename;
    document.body.appendChild(link);
...
async function showNpubQr() {
  ...
  try {
    const dataUrl = await import_qrcode.default.toDataURL(npub, { margin: 1, scale: 6 });
...
function populateKeys(keys = [], currentPub) {
  ...
  (keys || []).forEach((k) => {
    if (!k?.pubkey || !k?.nsec)
      return;
    if (!unique.find((u) => u.pubkey === k.pubkey || u.nsec === k.nsec))
      unique.push(k);
  });
  ...
  unique.forEach((k) => {
    ...
    if (currentPub && k.pubkey === currentPub)
      opt.selected = true;
    keySelect.appendChild(opt);
  });
  if (!keySelect.value && unique[0])
    keySelect.value = unique[0].nsec;
  if (currentPub) {
    keySelect.title = toNpub(currentPub);
    if (pubkeyLabel) {
      pubkeyLabel.textContent = `Current nPub: ${shortKey(currentPub)}`;
...
function checkRelayStatus(relay, dot) {
  ...
  const timer = setTimeout(() => finish("warn"), 4e3);
  try {
